...
function saveScope (done) {
  ...
  // If no scope was given, there is no need to ask the model to save the scope.
  // If you need to assign a default scope, then do so from the `getAccessToken`
  // model callback.
  if (!this.scope) return done();

  this.model.saveScope(this.accessToken, this.scope, function (err, scope) {
    if (err) return done(error('server_error', false, err));

    if (!scope) {
      return done(error('invalid_scope', 'The requested scope is invalid'));
    }
